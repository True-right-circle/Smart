...
class ViewControllerfirst4: UIViewController,XMLParserDelegate {
    
    @IBAction func img4(_ sender: Any) {
        UIApplication.shared.openURL(NSURL(string: infoer3[3])! as URL)
    ...
    {
        if i == 3{
            ds.text = startDater2[3]
        }
    }
    func loaddis(i: Int)
    {
        if i == 3{
            discount.text = discounter2[4]
            map = "https://map.naver.com/?query="+discounter2[4]+"&type=SITE_1&siteOrder="
        }
    }
    func loadtext(i: Int)
    {
        if i == 3{
            ptitle.text = titler2[3]
        }
    }
    func loadprice(i: Int)
    {
        if i == 3{
            price.text = pricer2[3]
        }
    }
