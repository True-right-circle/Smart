...
class ViewControllerfirst6: UIViewController,XMLParserDelegate {
    
    @IBAction func img6(_ sender: Any) {
        UIApplication.shared.openURL(NSURL(string: infoer3[5])! as URL)
    ...
    {
        if i == 5{
            discount.text = discounter2[6]
            map = "https://map.naver.com/?query="+discounter2[6]+"&type=SITE_1&siteOrder="
        }
    }
    func loadtext(i: Int)
    {
        if i == 5{
            ptitle.text = titler2[5]
        }
    }
    func loadprice(i: Int)
    {
        if i == 5{
            price.text = pricer2[5]
        }
    }
